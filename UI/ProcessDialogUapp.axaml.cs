...
{
    public partial class ProcessDialogUapp : Window, INotifyPropertyChanged
    {
        ...
        private void ScrollToPartition(FlashPartitionItem item)
        {
            try
            {
                ...
                if (index >= 0 && PartitionScrollViewer != null)
                {
                    ...
                    PartitionScrollViewer.Offset = new Vector(PartitionScrollViewer.Offset.X, Math.Max(0, scrollPosition - 100));
                }
            }
            catch (Exception ex)
            {
